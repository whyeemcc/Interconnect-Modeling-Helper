...
class Writer:
	...
	class WritePreRun:
		...
		def WriteBlock(self,base,corner,DF):
			...
			for name,value in Diel.iterrows():
				self.ws.write(locR + 2 + i, locC + 0, name, self.style)
				self.ws.write(locR + 2 + i, locC + 1, value['thick'], self.style)
				self.ws.write(locR + 2 + i, locC + 2, '(±'+str(value['t_varia'])+'%)', self.style)
				self.ws.write(locR + 2 + i, locC + 3, value['permittivity'], self.style)
				i += 1
			# write in conductor	
			i = 0
			for name,value in Cond.iterrows():
				self.ws.write(locR + 2 + i, locC + 4 + 0, name, self.style)
				self.ws.write(locR + 2 + i, locC + 4 + 1, value['thick'], self.style)
				self.ws.write(locR + 2 + i, locC + 4 + 2, '(±'+str(value['t_varia'])+'%)', self.style)
				self.ws.write(locR + 2 + i, locC + 4 + 3, value['width'], self.style)
				self.ws.write(locR + 2 + i, locC + 4 + 4,'(±'+str(value['w_varia'])+'%)', self.style)
				self.ws.write(locR + 2 + i, locC + 4 + 5, value['space'], self.style)
				self.ws.write(locR + 2 + i, locC + 4 + 6, value['Zmin'], self.style)
				i += 1 		
	...
	def WriteTechChar(self,base,corner,DF):
	...
	def WriteDics(self,base,corner,DF):
		"""
		Generate disc.pts
