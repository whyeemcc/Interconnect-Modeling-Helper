...
class LayerMessage:
	...
	def __init__(self,file):
		...
		if 'Interconnect' in sheet_list: 
			self.sheetname = 'Interconnect'
		elif 'interconnect' in sheet_list:
			self.sheetname = 'interconnect'		
		else:print("\n Warning: 请将所在 sheet 的名称改为 'Interconnect' 或 'interconnect'\n\n Press Enter to quit."); exit()
	...
	def Zmin(self,base,corner,flag):
		...
		Diel['thick'] = Diel['thick'] * (1 + DielCor[corner] * Diel['t_varia'] / 100) * 1e-4
		
		Cond['thick'] = Cond['thick'] * (1 + CondCor[corner] * Cond['t_varia'] / 100) * 1e-4
		Cond['width'] = Cond['width'] * (1 + CondCor[corner] * Cond['w_varia'] / 100)
		Cond['space'] = Cond['space'] - CondCor[corner] * Cond['width'] * Cond['w_varia'] / 100
		...
		for nameC,valueC in Cond.iterrows():
			...
			if nameC == Cond.index[0]:
				...
				height = sum(temp[:nameI]['thick'])
			# other conductors should consider the Cu/Al process
			else:
				height = sum(temp[:nameI]['thick']) - valueC['thick'] * flag
